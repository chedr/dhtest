...
print_help(char* cmd)
{
    ...
    fprintf(stdout, "    -r, --release\t\t\t\t# Releases obtained DHCP IP for "
                                    "corresponding MAC\n");
    fprintf(stdout, "    -L, --option51-lease_time [ Lease_time ] # Option 51. "
                                    "Requested lease time in secondes\n");
    fprintf(stdout, "    -I, --option50-ip\t[ IP_address ]\t# Option 50 IP address "
                                    "on DHCP discover\n");
    fprintf(
        stdout,
    ...
    fprintf(stdout,
                    "    -c, --custom-dhcp-option [ option_format ] # option_format - "
    ...
    fprintf(stdout, "    -T, --timeout\t\t[ cmd_timeout ]\t# Command returns "
                                    "within specified timout in seconds\n");
    fprintf(stdout, "    -b, --bind-ip\t\t\t\t# Listens on the obtained IP. "
                                    "Supported protocols - ARP and ICMP\n");
    fprintf(stdout, "    -k, --bind-timeout\t[ timeout ]\t# Listen timout in "
                                    "seconds. Default 3600 seconds\n");
    fprintf(stdout, "    -f, --bcast_flag\t\t\t# Sets broadcast flag on DHCP "
    ...
    fprintf(stdout, "    -p, --padding\t\t\t\t# Add padding to packet to be at "
                                    "least 300 bytes\n");
    fprintf(stdout, "    -P, --port\t\t[ port ]\t# Use port instead of 67\n");
    fprintf(stdout,
                    "    -g, --giaddr\t\t[ giaddr ]\t# Use giaddr instead of 0.0.0.0\n");
    fprintf(stdout, "    -u, --unicast\t\t[ ip ]\t\t# Unicast request, IP is "
                                    "optional. If not specified, the interface address will be "
                                    "used. \n");
    fprintf(stdout, "    -a, --nagios\t\t\t\t# Nagios output format. \n");
    fprintf(stdout, "    -S, --server\t\t[ address ]\t# Use server address instead "
                                    "of 255.255.255.255\n");
...
main(int argc, char* argv[])
{
    ...
    while (get_tmp < argc) {
        get_cmd =
            getopt_long(argc, argv, "m:i:v:t:bfVrpanNsju::T:P:g:S:I:o:k:L:h:d:c:",
        ...
        switch (get_cmd) {
            ...
            case 'd':
                if (strlen(optarg) > 256) {
                    fprintf(stdout,
                                    "FQDN domain name string size should be less than 256\n");
                    exit(2);
                }
                fqdn_flag = 1;
            ...
            case 'c':
                if (no_custom_dhcp_options == MAX_CUSTOM_DHCP_OPTIONS) {
                    fprintf(stdout, "MAX custom DHCP options reached. MAX custom DHCP "
                                                    "options supported : %d\n",
                ...
                */
                ...
                if ((sscanf((char*)optarg, "%u,str,%255[^\n]s", (uint32_t*)&option_no,
                    ...
                    custom_dhcp_options[option_index].option_type =
                        CUST_DHCP_OPTION_STRING;
                    custom_dhcp_options[option_index].option_len =
                        strlen((const char*)option_value);
                    memcpy(custom_dhcp_options[option_index].option_value, option_value,
                                 sizeof(custom_dhcp_options[option_index].option_value));

                } else if ((sscanf((char*)optarg, "%u,num,%u", (uint32_t*)&option_no,
                                                     &option_value_num)) == 2) {
                    no_custom_dhcp_options = no_custom_dhcp_options + 1;
                    option_index = no_custom_dhcp_options - 1;
                    ...
                    custom_dhcp_options[option_index].option_no = option_no;
                    custom_dhcp_options[option_index].option_type =
                        CUST_DHCP_OPTION_NUMBER;
                    custom_dhcp_options[option_index].option_len = 4; /*length of 4 byte*/
                    custom_dhcp_options[option_index].option_value_num =
                        htonl(option_value_num);
                    /*memcpy(custom_dhcp_options[option_index].option_value, option_value,
                     * sizeof(custom_dhcp_options[option_index].option_value));*/

                } else if ((sscanf((char*)optarg, "%u,hex,%254s", (uint32_t*)&option_no,
                                                     option_value)) == 2) {
                    if ((strlen(option_value) >= 255)) {
                        fprintf(stdout,
                                        "dhcp custom option value hex length is more than 254\n");
                        exit(2);
                    }
                    no_custom_dhcp_options = no_custom_dhcp_options + 1;
                    option_index = no_custom_dhcp_options - 1;
                    ...
                    for (tmp = 0; tmp < hex_length; tmp++) {
                        sscanf(&option_value[index], "%2X",
                                     (unsigned int*)&custom_dhcp_options[option_index]
                                         .option_value[tmp]);
                        index = index + 2;
                    }

                    /*print_buff(custom_dhcp_options[option_index].option_value,
                     * (hex_length/2));*/
                } else if ((sscanf((char*)optarg, "%u,ip,%s", (uint32_t*)&option_no,
                                                     option_value)) == 2) {
                    no_custom_dhcp_options = no_custom_dhcp_options + 1;
                    option_index = no_custom_dhcp_options - 1;
                ...
                if (custom_dhcp_options[option_index].option_type ==
                CUST_DHCP_OPTION_HEX) {
                        print_buff(custom_dhcp_options[option_index].option_value,
                ((strlen((const char *) option_value))/2));
                } else {
                        fprintf(stdout, "Option_value string - %s\n",
            ...
            case 'u':
                if (optarg) {
                    ...
                    if (!inet_pton(AF_INET, optarg, &out)) {
