 ...
 */
int
send_packet(int pkt_type)
{
    int ret;
    if (pkt_type == DHCP_MSGDISCOVER) {
        ret = sendto(
            sock_packet, dhcp_packet_disc,
            (l2_hdr_size + l3_hdr_size + l4_hdr_size + dhcp_hdr_size + dhopt_size), 0,
            (struct sockaddr*)&ll, sizeof(ll));
    } else if (pkt_type == DHCP_MSGREQUEST) {
        ret = sendto(
            sock_packet, dhcp_packet_request,
            (l2_hdr_size + l3_hdr_size + l4_hdr_size + dhcp_hdr_size + dhopt_size), 0,
            (struct sockaddr*)&ll, sizeof(ll));
    } else if (pkt_type == DHCP_MSGRELEASE) {
        ret = sendto(
            sock_packet, dhcp_packet_release,
            (l2_hdr_size + l3_hdr_size + l4_hdr_size + dhcp_hdr_size + dhopt_size), 0,
            (struct sockaddr*)&ll, sizeof(ll));
    } else if (pkt_type == ARP_SEND) {
        ret = sendto(sock_packet, arp_icmp_reply, 60, 0, (struct sockaddr*)&ll,
                                 sizeof(ll));
    } else if (pkt_type == ICMP_SEND) {
        ret = sendto(sock_packet, arp_icmp_reply,
                                 (l2_hdr_size + l3_hdr_size + ICMP_H + icmp_len), 0,
                                 (struct sockaddr*)&ll, sizeof(ll));
    }
    ...
    if (pkt_type == DHCP_MSGOFFER) {
        while (tval.tv_sec != 0) {
            ...
            if (retval == 0) {
                ...
                sock_len = sizeof(ll);
                ret =
                    recvfrom(sock_packet, dhcp_packet_offer, sizeof(dhcp_packet_offer), 0,
                                     (struct sockaddr*)&ll, (socklen_t*)&sock_len);
            }
            if (ret >= 60) {
        ...
        while (tval.tv_sec != 0) {
            ...
            if (retval == 0) {
                ...
                sock_len = sizeof(ll);
                ret = recvfrom(sock_packet, dhcp_packet_ack, sizeof(dhcp_packet_ack), 0,
                                             (struct sockaddr*)&ll, (socklen_t*)&sock_len);
            }
            if (ret >= 60) {
        ...
        while (tval_listen.tv_sec != 0) {
            ...
            if (retval == 0) {
                ...
                sock_len = sizeof(ll);
                ret = recvfrom(sock_packet, arp_icmp_packet, sizeof(arp_icmp_packet), 0,
                                             (struct sockaddr*)&ll, (socklen_t*)&sock_len);
            }
            if (ret >= 60) {
...
int
print_buff(uint8_t* buff, int size)
{
    int tmp;
    fprintf(stdout, "\n---------Buffer data---------\n");
 ...
 */
uint16_t
ipchksum(uint16_t* buff, int words)
{
    unsigned int sum, i;
    sum = 0;
    for (i = 0; i < words; i++) {
 ...
 */
uint16_t
icmpchksum(uint16_t* buff, int words)
{
    unsigned int sum, i;
 ...
 */
uint16_t
l4_sum(uint16_t* buff, int words, uint16_t* srcaddr, uint16_t* dstaddr,
             uint16_t proto, uint16_t len)
{
    ...
    if ((htons(len) % 2) == 1) {
        ...
        sum = sum + last_word;
        sum = sum + *(srcaddr) + *(srcaddr + 1) + *(dstaddr) + *(dstaddr + 1) +
                    proto + len;
        sum = (sum >> 16) + sum;
        return ~sum;
    } else {
...
build_option60_vci()
{
    ...
    memcpy((dhopt_buff + dhopt_size), &msgtype, 1);
    memcpy((dhopt_buff + dhopt_size + 1), &msglen, 1);
    memcpy((dhopt_buff + dhopt_size + 2), vci_buff,
                 strlen((const char*)vci_buff));
...
build_option12_hostname()
{
    ...
    memcpy((dhopt_buff + dhopt_size), &msgtype, 1);
    memcpy((dhopt_buff + dhopt_size + 1), &msglen, 1);
    memcpy((dhopt_buff + dhopt_size + 2), hostname_buff,
                 strlen((const char*)hostname_buff));
...
build_option81_fqdn()
{
    ...
    memcpy((dhopt_buff + dhopt_size + 5), fqdn_buff,
                 strlen((const char*)fqdn_buff));
...
{
    int option_index;
    for (option_index = 0; option_index < no_custom_dhcp_options;
             option_index++) {
        ...
        if (option_type == CUST_DHCP_OPTION_IP) {
            memcpy((dhopt_buff + dhopt_size + 2),
                         &custom_dhcp_options[option_index].option_value_ip, msglen);
        } else if (option_type == CUST_DHCP_OPTION_NUMBER) {
            memcpy((dhopt_buff + dhopt_size + 2),
                         &custom_dhcp_options[option_index].option_value_num, msglen);
        } else {
            memcpy((dhopt_buff + dhopt_size + 2),
                         custom_dhcp_options[option_index].option_value, msglen);
        }
        /*memcpy((dhopt_buff + dhopt_size + 2), hostname_buff, strlen((const char *)
         * hostname_buff));*/
...
build_dhpacket(int pkt_type)
{
    ...
    if (pkt_type == DHCP_MSGDISCOVER) {
        ...
        if (padding_flag && dhcp_packet_size < MINIMUM_PACKET_SIZE) {
            memset(dhopt_buff + dhopt_size, 0,
                         MINIMUM_PACKET_SIZE - dhcp_packet_size);
        ...
        iph->tos = l3_tos;
        iph->tot_len =
            htons(l3_hdr_size + l4_hdr_size + dhcp_hdr_size + dhopt_size);
        iph->id = 0;
        iph->frag_off = 0;
        iph->ttl = 64;
        iph->protocol = 17;
        ...
        iph->check =
            ipchksum((uint16_t*)(dhcp_packet_disc + l2_hdr_size), iph->ihl << 1);

        struct udphdr* uh =
            (struct udphdr*)(dhcp_packet_disc + l2_hdr_size + l3_hdr_size);
        ...
        struct dhcpv4_hdr* dhpointer =
            (struct dhcpv4_hdr*)(dhcp_packet_disc + l2_hdr_size + l3_hdr_size +
                                                     l4_hdr_size);
        ...
        uint8_t* dhopt_pointer =
            (uint8_t*)(dhcp_packet_disc + l2_hdr_size + l3_hdr_size + l4_hdr_size +
                                 dhcp_hdr_size);
        memcpy(dhopt_pointer, dhopt_buff, dhopt_size);

        /* UDP checksum is done here */
        uh->check = l4_sum(
            (uint16_t*)(dhcp_packet_disc + l2_hdr_size + l3_hdr_size),
            ((dhcp_hdr_size + dhopt_size + l4_hdr_size) / 2), (uint16_t*)&iph->saddr,
            (uint16_t*)&iph->daddr, htons(l4_proto), htons(l4_len));
    }
    if (pkt_type == DHCP_MSGREQUEST) {
        ...
        iph->daddr = inet_addr((const char*)server_addr);
        iph->check =
            ipchksum((uint16_t*)(dhcp_packet_request + l2_hdr_size), iph->ihl << 1);

        struct udphdr* uh =
            (struct udphdr*)(dhcp_packet_request + l2_hdr_size + l3_hdr_size);
        uh->source = htons(port + 1);
        uh->dest = htons(port);
        uint16_t l4_proto = 17;
        uint16_t l4_len = (l4_hdr_size + dhcp_hdr_size + dhopt_size);
        ...
        struct dhcpv4_hdr* dhpointer =
            (struct dhcpv4_hdr*)(dhcp_packet_request + l2_hdr_size + l3_hdr_size +
                                                     l4_hdr_size);
        dhpointer->dhcp_opcode = DHCP_REQUEST;
        dhpointer->dhcp_htype = ARPHRD_ETHER;
        dhpointer->dhcp_hlen = ETHER_ADDR_LEN;
        ...
        /* DHCP option buffer is copied here to DHCP packet */
        uint8_t* dhopt_pointer =
            (uint8_t*)(dhcp_packet_request + l2_hdr_size + l3_hdr_size + l4_hdr_size +
                                 dhcp_hdr_size);
        memcpy(dhopt_pointer, dhopt_buff, dhopt_size);

        /* UDP checksum is done here */
        uh->check = l4_sum(
            (uint16_t*)(dhcp_packet_request + l2_hdr_size + l3_hdr_size),
            ((dhcp_hdr_size + dhopt_size + l4_hdr_size) / 2), (uint16_t*)&iph->saddr,
            (uint16_t*)&iph->daddr, htons(l4_proto), htons(l4_len));
    }
    if (pkt_type == DHCP_MSGRELEASE) {
        ...
        iph->check =
            ipchksum((uint16_t*)(dhcp_packet_release + l2_hdr_size), iph->ihl << 1);

        struct udphdr* uh =
            (struct udphdr*)(dhcp_packet_release + l2_hdr_size + l3_hdr_size);
        uh->source = htons(port + 1);
        uh->dest = htons(port);
        uint16_t l4_proto = 17;
        uint16_t l4_len = (l4_hdr_size + dhcp_hdr_size + dhopt_size);
        ...
        struct dhcpv4_hdr* dhpointer =
            (struct dhcpv4_hdr*)(dhcp_packet_release + l2_hdr_size + l3_hdr_size +
                                                     l4_hdr_size);
        dhpointer->dhcp_opcode = DHCP_REQUEST;
        dhpointer->dhcp_htype = ARPHRD_ETHER;
        dhpointer->dhcp_hlen = ETHER_ADDR_LEN;
        ...
        /* DHCP option buffer is copied here to DHCP packet */
        uint8_t* dhopt_pointer =
            (uint8_t*)(dhcp_packet_release + l2_hdr_size + l3_hdr_size + l4_hdr_size +
                                 dhcp_hdr_size);
        memcpy(dhopt_pointer, dhopt_buff, dhopt_size);

        /* UDP checksum is done here */
        uh->check = l4_sum(
            (uint16_t*)(dhcp_packet_release + l2_hdr_size + l3_hdr_size),
            ((dhcp_hdr_size + dhopt_size + l4_hdr_size) / 2), (uint16_t*)&iph->saddr,
            (uint16_t*)&iph->daddr, htons(l4_proto), htons(l4_len));
    }
    return 0;
...
build_packet(int pkt_type)
{
    ...
    if (pkt_type == ARP_SEND) {
        ...
        struct icmp_hdr* ich =
            (struct icmp_hdr*)(arp_icmp_reply + l2_hdr_size + l3_hdr_size);
        ...
        memcpy((((uint8_t*)&ich->seq) + 1), (((uint8_t*)&icmp_hg->seq) + 1),
                     (icmp_len + 1));
        iph->tot_len = htons((l3_hdr_size + ICMP_H + icmp_len));
        iph->check =
            ipchksum((uint16_t*)(arp_icmp_reply + l2_hdr_size), iph->ihl << 1);
        ich->icmp_sum =
            icmpchksum((uint16_t*)(arp_icmp_reply + l2_hdr_size + l3_hdr_size),
                                 ((icmp_len + ICMP_H) / 2));
    }
    return 0;
}
 ...
 */
int
check_packet(int pkt_type)
{

    uint8_t* dhopt_pointer_tmp;
    if (pkt_type == DHCP_MSGOFFER && vlan != 0) {
        map_all_layer_ptr(DHCP_MSGOFFER);
        if ((ntohs(vlan_hg->vlan_priority_c_vid) & VLAN_VIDMASK) == vlan &&
                ntohs(vlan_hg->vlan_tpi) == ETHERTYPE_VLAN && iph_g->protocol == 17 &&
                uh_g->source == htons(port) &&
                (uh_g->dest == htons(port + 1) || uh_g->dest == htons(port))) {
            dhopt_pointer_tmp = dhopt_pointer_g;
            while (*(dhopt_pointer_tmp) != DHCP_END) {
                if (*(dhopt_pointer_tmp) == DHCP_MESSAGETYPE &&
                        *(dhopt_pointer_tmp + 2) == DHCP_MSGOFFER &&
                        htonl(dhcph_g->dhcp_xid) == dhcp_xid) {
                    return DHCP_OFFR_RCVD;
                    break;
                }
        ...
        if ((ntohs(vlan_hg->vlan_priority_c_vid) & VLAN_VIDMASK) == vlan &&
            ...
            while (*(dhopt_pointer_tmp) != DHCP_END) {
                if (*(dhopt_pointer_tmp) == DHCP_MESSAGETYPE &&
                        *(dhopt_pointer_tmp + 2) == DHCP_MSGACK &&
                        htonl(dhcph_g->dhcp_xid) == dhcp_xid) {
                    return DHCP_ACK_RCVD;
                    break;
                }
                if (*(dhopt_pointer_tmp) == DHCP_MESSAGETYPE &&
                        *(dhopt_pointer_tmp + 2) == DHCP_MSGNACK &&
                        htonl(dhcph_g->dhcp_xid) == dhcp_xid) {
                    return DHCP_NAK_RCVD;
                    break;
                }
        ...
        map_all_layer_ptr(DHCP_MSGOFFER);
        if (eth_hg->ether_type == htons(ETHERTYPE_IP) && iph_g->protocol == 17 &&
                uh_g->source == htons(port) &&
                (uh_g->dest == htons(port + 1) || uh_g->dest == htons(port))) {
            dhopt_pointer_tmp = dhopt_pointer_g;
            while (*(dhopt_pointer_tmp) != DHCP_END) {
        ...
        if (!vlan) {
            ...
            if ((ntohs(arp_hg->ar_op)) == ARPOP_REQUEST &&
                    (htonl(ip_address)) == (*((uint32_t*)(arp_hg->target_ip)))) {
                return ARP_RCVD;
            }
        } else if (vlan &&
                             ntohs(vlan) == (vlan_hg->vlan_priority_c_vid & VLAN_VIDMASK)) {
            if ((ntohs(arp_hg->ar_op)) == ARPOP_REQUEST &&
                    (htonl(ip_address)) == (*((uint32_t*)(arp_hg->target_ip)))) {
                if (json_flag) {
                    if (!json_first) {
        ...
        map_all_layer_ptr(ICMP_MAP);
        if (!vlan) {
            if ((ntohs(eth_hg->ether_type)) == ETHERTYPE_IP && iph_g->protocol == 1 &&
                    ip_address == ntohl(iph_g->daddr) &&
                    icmp_hg->icmp_type == ICMP_ECHO) {
                return ICMP_RCVD;
            }
        } else if (vlan &&
                             ntohs(vlan) == (vlan_hg->vlan_priority_c_vid & VLAN_VIDMASK)) {
            if ((ntohs(vlan_hg->vlan_len)) == ETHERTYPE_IP && iph_g->protocol == 1 &&
                    ip_address == ntohl(iph_g->daddr) &&
                    icmp_hg->icmp_type == ICMP_ECHO) {
                return ICMP_RCVD;
            }
    ...
    if (pkt_type == DHCP_MSGOFFER) {
        ...
        if (json_flag) {
            ...
            fprintf(stdout, "{\"msg\":\"DHCP offer details\","
                                            ...
                                            "\"result-ip\":\"%s\","
                                            "\"result-next-srv\":\"%s\"",
                            get_ip_str(dhcph_g->dhcp_yip), get_ip_str(dhcph_g->dhcp_sip));

            if (dhcph_g->dhcp_gip) {
                fprintf(stdout, ",\"result-relay-agent\":\"%s\"",
        ...
        switch (*(dhopt_pointer_g)) {
            ...
            case DHCP_LEASETIME:
                if (json_flag) {
                    ...
                    fprintf(
                        stdout, "{\"msg\":\"Lease time - %d Days %d Hours %d Minutes\","
                                        "\"result\":\"info\","
                                        "\"result-type\":\"option\","
            ...
            case DHCP_FQDN: {
                ...
                if (json_flag) {
                    ...
                    fprintf(stdout, "{\"msg\":\"FQDN Client name - %s\","
                                                    ...
                                                    "\"result-router\":\"%s\"}",
                                    fqdn_client_name, fqdn_client_name);
                } else {
            ...
            default:
                if (json_flag) {
                    ...
                    fprintf(stdout, "Option no - %d, option length - %d",
                                    *dhopt_pointer_g, *(dhopt_pointer_g + 1));
 ...
 */
int
map_all_layer_ptr(int pkt_type)
{
    if (pkt_type == DHCP_MSGOFFER && vlan != 0) {
        ...
        dhcph_g = (struct dhcpv4_hdr*)(dhcp_packet_offer + l2_hdr_size +
                                                                     l3_hdr_size + l4_hdr_size);
        dhopt_pointer_g = (uint8_t*)(dhcp_packet_offer + l2_hdr_size + l3_hdr_size +
                                                                 l4_hdr_size + sizeof(struct dhcpv4_hdr));
        ...
        dhcph_g = (struct dhcpv4_hdr*)(dhcp_packet_ack + l2_hdr_size + l3_hdr_size +
                                                                     l4_hdr_size);
        dhopt_pointer_g = (uint8_t*)(dhcp_packet_ack + l2_hdr_size + l3_hdr_size +
                                                                 l4_hdr_size + sizeof(struct dhcpv4_hdr));
    } else if (pkt_type == DHCP_MSGACK && vlan == 0) {
...
log_dhinfo()
{
    ...
    if (!vlan) {
        ...
        fprintf(dh_file, "Host_mac: %02X:%02X:%02X:%02X:%02X:%02X\n",
                        eth_hg->ether_shost[0], eth_hg->ether_shost[1],
                        eth_hg->ether_shost[2], eth_hg->ether_shost[3],
        ...
        fprintf(dh_file, "Server_id: %s\n", get_ip_str(server_id));
        fprintf(dh_file, "Host_mac: %02X:%02X:%02X:%02X:%02X:%02X\n",
                        vlan_hg->vlan_shost[0], vlan_hg->vlan_shost[1],
                        vlan_hg->vlan_shost[2], vlan_hg->vlan_shost[3],
...
{
    FILE* dh_file;
    uint8_t aux_dmac[ETHER_ADDR_LEN];
    char mac_tmp[20], acq_ip_tmp[20], serv_id_tmp[20], dmac_tmp[20],
        ip_listen_tmp[10];
    pid_t dh_pid;
    ...
    fscanf(dh_file, "Client_mac: %s\nAcquired_ip: %s\nServer_id: %s\n\
			Host_mac: %2X:%2X:%2X:%2X:%2X:%2X\nIP_listen: %s Pid: %d",
                 mac_tmp, acq_ip_tmp, serv_id_tmp, (uint32_t*)&aux_dmac[0],
                 (uint32_t*)&aux_dmac[1], (uint32_t*)&aux_dmac[2],
                 (uint32_t*)&aux_dmac[3], (uint32_t*)&aux_dmac[4],
                 (uint32_t*)&aux_dmac[5], ip_listen_tmp, &dh_pid);
...
int
print_dhoption(uint8_t* buff, int size)
{
    int tmp;
    fprintf(stdout, "\n    OPTION data (HEX)\n        ");
    for (tmp = 0; tmp < size; tmp++) {
 ...
 */
int
get_if_mac_address(char* if_name, uint8_t* mac_address)
{
    struct ifreq ifr;
    int sockfd;
...
char*
mac2str(uint8_t* mac_addr)
{
    static char str[25];

    snprintf(str, 25, "%02x:%02x:%02x:%02x:%02x:%02x", mac_addr[0], mac_addr[1],
...
int
str2mac(char* str, uint8_t* mac_addr)
{
    ...
    local_mac_str[2] = local_mac_str[5] = local_mac_str[8] = local_mac_str[11] =
        local_mac_str[14] = 0x00;
